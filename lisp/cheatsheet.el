...
(defface svg-tag-keyboard-face
  '((t :foreground "#333333"
       ...
       :weight bold
       :height 110))
  "Face for keyboard bindings tag"
  :group nil)
...
(setq svg-tag-horizontal-offset 2)
...
(defun svg-tag-keyboard (text)
  (svg-tag-make (substring text 1 -1) 'svg-tag-keyboard-face 0 0 2))
...
(defun cheatsheet-add-group (group &rest cheats)
  ...
  (mapcar #'(lambda (cheat)
          ...
          cheats))
(defcustom cheatsheet-display-action '(cheatsheet--display-fn)
  "Displays the cheatsheet to the side"
  :type 'sexp
  :group 'cheatsheet)


(defun cheatsheet--display-fn(buffer _alist)
  (display-buffer-in-side-window buffer `((side . right)

					  (window-width . fit-window-to-buffer))))
(defvar cheatsheet--buffer nil)
(defvar cheatsheet--window nil)

(defun cheatsheet--create-buffer ()
...
(defun cheatsheet-show ()
  ...
  (save-window-excursion
    (setq buffer (cheatsheet--create-buffer)))
  (setq window
	(select-window
	 (display-buffer buffer cheatsheet-display-action)))
...
(define-derived-mode cheatsheet-mode special-mode "Cheat Sheet"
